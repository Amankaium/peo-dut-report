...
class TransportIdRealcomView(View):
    def get(self, request):
        return render(request, 'core/id_realcom_transport_add.html')
    def post(self, request):
        excel_file = request.FILES["excel_file"]
        ...
        for row in page:
            ...
            new_id_realcom, created = Transport.objects.get_or_create(id_realcom=id_realcom)
            if created:
                created_qty += 1
            print(f'id_realcom: {id_realcom}, created: {created}')
        messages.success(request, f"Добавлено {created_qty} записей")
        return render(request, 'core/id_realcom_transport_add.html', context)

class TransportIdRealcomGetView(View):
    def get(self, request):
        context = {}
        transports = Transport.objects.all()
